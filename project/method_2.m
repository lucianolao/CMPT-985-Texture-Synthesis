function [I] = method_2(img, size_block, size_overlap, out_height, out_width, tolerance, n_best, bool_seam_cut, bool_transfer, img_transfer, alpha)
    ...
    for i = 1 : size_non_overlap : out_height
        for j = 1 : size_non_overlap : out_width
            ...
            elseif i==1 && j > 1
                % left overlap
                I(i:i+size_block-1, j:j+size_block-1, :) = pickBlock(img, current_block, size_overlap, tolerance, n_best, "left", bool_seam_cut, bool_transfer, img_transfer, alpha, img_squared_left, img_squared_top, img_squared_top_minus_left);
            elseif i > 1 && j==1
                % top overlap
                I(i:i+size_block-1, j:j+size_block-1, :) = pickBlock(img, current_block, size_overlap, tolerance, n_best, "top", bool_seam_cut, bool_transfer, img_transfer, alpha, img_squared_left, img_squared_top, img_squared_top_minus_left);
            else
                % double overlap
                I(i:i+size_block-1, j:j+size_block-1, :) = pickBlock(img, current_block, size_overlap, tolerance, n_best, "double", bool_seam_cut, bool_transfer, img_transfer, alpha, img_squared_left, img_squared_top, img_squared_top_minus_left);
            end
        end
    end
